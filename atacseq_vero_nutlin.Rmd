...
atac_counts <- read.table(file="datafiles/vero_atacseq_nutlin_count.filt.txt", header=F, skip=1)
atac_counts <- atac_counts[!duplicated(atac_counts[,1]),]
row.names(atac_counts) <- atac_counts[,1]
atac_counts <- atac_counts[,-c(1:8)]

# read in ATAC-seq library information
sampleinfo <- read.csv(file="sample_info.csv")

sampleinfo <- sampleinfo[73:76,]
row.names(sampleinfo) <- sampleinfo$Full.Name
...
```{r, eval=T, fig.width=1.5, fig.height=0.7}
...
```{r, eval=F}
atac_results_vero_nut <- data.frame(lfcShrink(atac_dds, coef=2))
```

```{r, eval=T, fig.width=1.5, fig.height=1.5}
ggplot(atac_results_vero_nut, aes(x=log2FoldChange, y=-log10(padj))) + geom_point(pch=20, color="lightgray", alpha=0.5, data=subset(atac_results_vero_nut, padj > 0.05)) + geom_point(pch=20, color="skyblue", data=subset(atac_results_vero_nut, padj < 0.05 & log2FoldChange > 0), alpha=0.5) + geom_point(pch=20, color="indianred", data=subset(atac_results_vero_nut, padj < 0.05 & log2FoldChange < 0), alpha=0.5) + theme_classic() + geom_hline(yintercept=0, lty=3) +
  geom_hline(yintercept=-log10(0.05), lty=3, color="red") + geom_vline(xintercept=0, lty=3, color="red") + ylab("-Log10 FDR") + xlab("ATAC-seq Log2 Fold Change\nVeroE6 Nutlin-3a vs. DMSO") #+ xlim(c(-10,10)) + ylim(c(0,300))
```

```{r, eval=T}
write.table(atac_results_vero_nut, file="atac_results_vero_nut.txt", sep='\t', quote=F)
```
